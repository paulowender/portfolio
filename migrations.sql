...
CREATE POLICY IF NOT EXISTS "Users can view their own company"
...
CREATE POLICY IF NOT EXISTS "Users can view their own AI config"
  ON "ai_configs" FOR SELECT
  TO authenticated
  USING (user_id = auth.uid()::text);

CREATE POLICY IF NOT EXISTS "Users can create their own AI config"
  ON "ai_configs" FOR INSERT
  TO authenticated
  WITH CHECK (user_id = auth.uid()::text);

CREATE POLICY IF NOT EXISTS "Users can update their own AI config"
  ON "ai_configs" FOR UPDATE
  TO authenticated
  USING (user_id = auth.uid()::text);

CREATE POLICY IF NOT EXISTS "Users can delete their own AI config"
  ON "ai_configs" FOR DELETE
  TO authenticated
  USING (user_id = auth.uid()::text);

-- Create policies for messaging_configs table
CREATE POLICY IF NOT EXISTS "Users can view their own messaging config"
  ON "messaging_configs" FOR SELECT
  TO authenticated
  USING (user_id = auth.uid()::text);

CREATE POLICY IF NOT EXISTS "Users can create their own messaging config"
  ON "messaging_configs" FOR INSERT
  TO authenticated
  WITH CHECK (user_id = auth.uid()::text);

CREATE POLICY IF NOT EXISTS "Users can update their own messaging config"
  ON "messaging_configs" FOR UPDATE
  TO authenticated
  USING (user_id = auth.uid()::text);

CREATE POLICY IF NOT EXISTS "Users can delete their own messaging config"
